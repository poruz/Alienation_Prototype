...
public class LopaInteractAI : MonoBehaviour
{
    ...
    IEnumerator Speak()
    {
        oldAudio.Stop();
        newAudio.Play();
        player.GetComponent<PlayerMovementAndAnimControlCave>().enabled = false;
        ...
        mTextBox.SetActive(false);
        yield return new WaitForSeconds(1.0f);
        player.GetComponent<PlayerMovementAndAnimControlCave>().enabled = true;
        mAlreadySpeaking = false;
    }
